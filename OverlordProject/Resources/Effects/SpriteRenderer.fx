...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float4 col, float2 texCoord, float rotation, float2 rotCosSin, float2 offset, float2 pivotOffset)
{
    ...
    float3 offsetPosition = pos - float3(offset.x + pivotOffset.x, offset.y + pivotOffset.y, 0.0f);
    ...
    float rotPosX = offsetPosition.x * rotCosSin.x - offsetPosition.y * rotCosSin.y;
    float rotPosY = offsetPosition.y * rotCosSin.x + offsetPosition.x * rotCosSin.y;
    ...
    geomData.Position = mul(float4(rotPosX, rotPosY, pos.z, 1.0f), gTransform);
    geomData.Color = col;
