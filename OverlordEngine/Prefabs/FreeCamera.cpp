...
void FreeCamera::Update(const SceneContext& sceneContext)
{
	...
	if (m_pCamera->IsActive())
	{
		...
		int Up = InputManager::IsKeyboardKey(InputState::down, 'E') ? 1 : 0;
		if (Up == 0) Up = -(InputManager::IsKeyboardKey(InputState::down, 'Q') ? 1 : 0);

		const XMFLOAT3 UpDown{0, static_cast<float>(Up) * currSpeed * elapsedTime , 0};

		currPos += XMLoadFloat3(&UpDown);
